...
export const buildColStyles = ({
  ...
  el: customizeTopLevel(
    [
      // We originally tried using string templates for these, but we have to write them all out
      // by hand because of Tailwind 3's JIT mode
      {
        'p-8': gutter,
        'w-1/12': xs === '1',
        'w-2/12': xs === '2',
 ...
 */
export function Column({ children, className, custom, md, lg, sm, xs }: ColumnProps) {
  const context = useContext(RowContext);
  const styles = useMemo(
    () => buildColStyles({ className, custom, gutter: context.gutter, md, lg, sm, xs }),
    [className, custom, context.gutter, md, lg, sm, xs]
  );
